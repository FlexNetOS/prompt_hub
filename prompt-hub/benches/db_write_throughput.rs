...
fn bench_insert_prompt(c: &mut Criterion) {
    ...
    let hub = rt.block_on(async {
        PromptHub::new(&db_path, config).await.unwrap()
    });
...
fn bench_search_fast(c: &mut Criterion) {
    ...
    c.bench_function("search_fast_100_prompts", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = hub.search("test", SearchMode::Fast, SearchFilters::default(), Pagination::default()).await;
        });
    });
}
...
fn bench_search_smart(c: &mut Criterion) {
    ...
    group.bench_function("smart_50_prompts", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = hub.search("explain", SearchMode::Smart, SearchFilters::default(), Pagination::default()).await;
        });
    });
    group.finish();
}
...
fn bench_lock_unlock(c: &mut Criterion) {
    ...
    c.bench_function("lock_acquire", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = hub.lock(prompt_id, &identity, std::time::Duration::from_secs(60)).await;
        });
    });
}

criterion_group!(benches, bench_insert_prompt, bench_search_fast, bench_search_smart, bench_lock_unlock);
